...
func (m *TrainingStatusModel) Delete(id int64) error {
	if id < 1 {
		return ErrRecordNotFound
	}
...
func (m *TrainingStatusModel) GetByName(name string) (*TrainingStatus, error) {
	query := `SELECT id, status FROM training_status WHERE status = $1`
