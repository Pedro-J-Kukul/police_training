...
func (m RankModel) Insert(rank *Rank) error {
	...
	if err := m.DB.QueryRowContext(ctx, query, rank.Rank, rank.Code, rank.AnnualTrainingHoursRequired).Scan(&rank.ID); err != nil {
		switch {
...
func (m RankModel) Get(id int64) (*Rank, error) {
	...
	err := m.DB.QueryRowContext(ctx, query, id).Scan(&rank.ID, &rank.Rank, &rank.Code, &rank.AnnualTrainingHoursRequired)
	if err != nil {
		switch {
...
func (m RankModel) GetAll(name string, code string, filters Filters) ([]*Rank, MetaData, error) {
	...
	for rows.Next() {
		var rank Rank
		if err := rows.Scan(&totalRecords, &rank.ID, &rank.Rank, &rank.Code, &rank.AnnualTrainingHoursRequired); err != nil {
			return nil, MetaData{}, err
		}
...
func (m RankModel) Update(rank *Rank) error {
	...
	if err := m.DB.QueryRowContext(ctx, query, rank.Rank, rank.Code, rank.AnnualTrainingHoursRequired, rank.ID).Scan(&rank.Rank, &rank.Code, &rank.AnnualTrainingHoursRequired); err != nil {
		switch {
		case errors.Is(err, sql.ErrNoRows):
