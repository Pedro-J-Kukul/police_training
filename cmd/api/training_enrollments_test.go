...
func createTestEnrollment(t *testing.T) *data.TrainingEnrollment {
	...
	existing, err := testApp.models.TrainingEnrollment.GetByOfficerAndSession(officerID, sessionID)
	if err == nil {
		testApp.models.TrainingEnrollment.Delete(existing.ID)
	}
...
func TestEnrollmentWorkflow(t *testing.T) {
	...
	if err == nil {
		testApp.models.TrainingEnrollment.Delete(existing.ID)
		t.Logf("Step: Deleted existing enrollment for officer %d in session %d", officerID, sessionID)
	}
	...
	if rec.Result().StatusCode != http.StatusCreated {
		t.Fatalf("Enrollment creation failed with status %d: %s", rec.Result().StatusCode, string(body))
	}
