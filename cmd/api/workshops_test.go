...
func TestCreateWorkshopHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedStatus: http.StatusConflict, // Change from 422 to 409
			checkResponse: func(t *testing.T, res *http.Response) {
...
func TestListWorkshopsHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			checkResponse: func(t *testing.T, res *http.Response) {
				...
				for i, workshopInterface := range workshops {
					...
					if catID, ok := workshop["category_id"].(float64); !ok || int64(catID) != categoryID {
		...
		},
		{
			...
			queryParams:    "?workshop_name=" + url.QueryEscape(getSeededWorkshop(t).WorkshopName),
			expectedStatus: http.StatusOK,
			checkResponse: func(t *testing.T, res *http.Response) {
